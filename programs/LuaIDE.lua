...
local function modRead(properties)
	...
	gpu.cursBlink(true)
	while true do
		local e, but, x, y, p4, p5 = iop.pullEvt()
...
local function centerPrint(text, ny)
	if type(text) == "table" then for _, v in pairs(text) do centerPrint(v) end
		...
		local w, h = term.getSize()
		gpu.cursPos(w/2 - text:len()/2 + (#text % 2 == 0 and 1 or 0), ny or y)
		print(text)
	end
end

local function title(t)
	gpu.setTxt(colors[theme.textColor])
	gpu.bg(colors[theme.background])
	term.clear()

	gpu.bg(colors[theme.backgroundHighlight])
	for i = 2, 4 do gpu.cursPos(1, i) term.clearLine() end
	gpu.cursPos(3, 3)
	term.write(t)
end
...
local function centerRead(wid, begt)
	...
	gpu.setTxt(colors[theme.textColor])
	gpu.bg(colors[theme.promptHighlight])
	for i = 8, 10 do
		gpu.cursPos(w/2 - wid/2, i)
		term.write(string.rep(" ", wid))
	end

	if isAdvanced() then
		gpu.bg(colors[theme.errHighlight])
		for i = 13, 15 do
			gpu.cursPos(w/2 - wid/2 + 1, i)
			term.write(string.rep(" ", 10))
		end
		gpu.cursPos(w/2 - wid/2 + 2, 14)
		term.write("> Cancel")
	end

	gpu.bg(colors[theme.promptHighlight])
	gpu.cursPos(w/2 - wid/2 + 1, 9)
	term.write("> " .. begt)
...
local function prompt(list, dir, isGrid)
	local function draw(sel)
		for i, v in ipairs(list) do
			if i == sel then gpu.bg(v.highlight or colors[theme.promptHighlight])
			else gpu.bg(v.bg or colors[theme.prompt]) end
			gpu.setTxt(v.tc or colors[theme.textColor])
			for i = -1, 1 do
				gpu.cursPos(v[2], v[3] + i)
				term.write(string.rep(" ", v[1]:len() + 4))
			end

			gpu.cursPos(v[2], v[3])
			if i == sel then
				gpu.bg(v.highlight or colors[theme.promptHighlight])
				term.write(" > ")
			else term.write(" - ") end
			term.write(v[1] .. " ")
		end
...
local function scrollingPrompt(list)
	local function draw(items, sel, loc)
		for i, v in ipairs(items) do
			...
			if i == sel then gpu.bg(bghigh)
			else gpu.bg(bg) end
			gpu.setTxt(colors[theme.textColor])
			for x = -1, 1 do
				gpu.cursPos(3, (i * 4) + x + 4)
	...
	while true do
		...
		elseif e == "mouse_scroll" then
			iop.queueEvent("key", key == -1 and 200 or 208)
...
function monitorKeyboardShortcuts()
	...
	while true do
		local event, char = iop.pullEvt()
		if event == "key" and (char == 42 or char == 52) then
			shiftPressed = true
			tb = cpu.startTimer(keyboardShortcutTimeout)
		elseif event == "key" and (char == 29 or char == 157 or char == 219 or char == 220) then
			allowEditorEvent = false
			allowChar = true
			ta = cpu.startTimer(keyboardShortcutTimeout)
		elseif event == "key" and allowChar then
			local name = nil
			for k, v in pairs(keys) do
				if v == char then
					if shiftPressed then iop.queueEvent("shortcut", "ctrl shift", k:lower())
					else iop.queueEvent("shortcut", "ctrl", k:lower()) end
					sleep(0.005)
					allowEditorEvent = true
				end
			end
			if shiftPressed then iop.queueEvent("shortcut", "ctrl shift", char)
			else iop.queueEvent("shortcut", "ctrl", char) end
...
local function saveFile(path, lines)
	local dir = path:sub(1, path:len() - iop.getName(path):len())
	if not fs.exists(dir) then iop.mkDir(dir) end
...
local function viewErrorHelp(e)
	...
	gpu.bg(colors[theme.err])
	for i = 6, 8 do
		gpu.cursPos(5, i)
		term.write(string.rep(" ", 35))
	end

	gpu.bg(colors[theme.prompt])
	for i = 10, 18 do
		gpu.cursPos(5, i)
...
local function run(path, lines, useArgs)
	local ar = {}
	if useArgs then
		title("LuaIDE - Run " .. iop.getName(path))
		local s = centerRead(w - 13, iop.getName(path) .. " ")
	...
	print("\n")
	if err then
		if isAdvanced() then gpu.setTxt("red") end
	...
	iop.queueEvent(event_distract)
	iop.pullEvt()

	if err then
		if curLanguage == languages.lua and err:find("]") then
			err = iop.getName(path) .. err:sub(err:find("]", 1, true) + 1, -1)
		end
...
local function setsyntax()
	...
	term.write(opts[sel])
	while true do
		local e, but, x, y = iop.pullEvt("key")
		if but == 203 then
...
--	["Settings"] = function() end,
	...
	["Start of Line ^+<"] = function() iop.queueEvent("key", 199) end,
	["End of Line   ^+>"] = function() iop.queueEvent("key", 207) end,
...
local lastEventClock = cpu.clock()

local function attemptToHighlight(line, regex, col)
	local match = string.match(line, regex)
	if match then
		if type(col) == "number" then gpu.setTxt(col)
		elseif type(col) == "function" then gpu.setTxt(col(match)) end
		term.write(match)
		gpu.setTxt(colors[theme.textColor])
...
local function draw()
	-- Menu
	gpu.setTxt(colors[theme.textColor])
	gpu.bg(colors[theme.editorBackground])
	term.clear()
	drawMenu()
	...
	for i = 1, edh do
		...
		if a then
			...
			gpu.cursPos(1, i + offy)
			gpu.bg(colors[theme.editorBackground])
			if scrolly + i == y then
				if scrolly + i == liveErr.line and cpu.clock() - lastEventClock > 3 then
					gpu.bg(colors[theme.editorErrorHighlight])
				else gpu.bg(colors[theme.editorLineHightlight]) end
				term.clearLine()
			elseif scrolly + i == liveErr.line then
				gpu.bg(colors[theme.editorError])
				term.clearLine()
			end

			gpu.cursPos(1 - scrollx + offx, i + offy)
			if scrolly + i == y then
				if scrolly + i == liveErr.line and cpu.clock() - lastEventClock > 3 then
					gpu.bg(colors[theme.editorErrorHighlight])
				else gpu.bg(colors[theme.editorLineHightlight]) end
			elseif scrolly + i == liveErr.line then gpu.bg(colors[theme.editorError])
			else gpu.bg(colors[theme.editorBackground]) end
			...
			if scrolly + i == y then
				if scrolly + i == liveErr.line and cpu.clock() - lastEventClock > 3 then
					gpu.bg(colors[theme.editorError])
				else gpu.bg(colors[theme.editorLineNumbersHighlight]) end
			elseif scrolly + i == liveErr.line then
				gpu.bg(colors[theme.editorErrorHighlight])
			else gpu.bg(colors[theme.editorLineNumbers]) end
			term.write(ln)
		end
	end
	gpu.cursPos(x - scrollx + offx, y - scrolly + offy)
end
...
local function drawLine(...)
	...
	for _, ly in pairs(ls) do
		...
		if a then
			...
			gpu.cursPos(1, (ly - scrolly) + offy)
			gpu.bg(colors[theme.editorBackground])
			if ly == y then
				if ly == liveErr.line and cpu.clock() - lastEventClock > 3 then
					gpu.bg(colors[theme.editorErrorHighlight])
				else gpu.bg(colors[theme.editorLineHightlight]) end
			...
			gpu.cursPos(1 - scrollx + offx, (ly - scrolly) + offy)
			if ly == y then
				if ly == liveErr.line and cpu.clock() - lastEventClock > 3 then
					gpu.bg(colors[theme.editorErrorHighlight])
				else gpu.bg(colors[theme.editorLineHightlight]) end
			elseif ly == liveErr.line then gpu.bg(colors[theme.editorError])
			else gpu.bg(colors[theme.editorBackground]) end
...
local function executeMenuItem(a, path)
	if type(a) == "string" and menuFunctions[a] then
		...
		if type(opt) == "string" then gpu.cursBlink(false) return opt end
...
local function edit(path)
	...
	-- Clocks
	local autosaveClock = cpu.clock()
	local scrollClock = cpu.clock() -- To prevent redraw flicker
	local liveErrorClock = cpu.clock()
	local hasScrolled = false

	-- Draw
	draw()
	gpu.cursPos(x + offx, y + offy)
	gpu.cursBlink(true)
	
	-- Main loop
	local tid = cpu.startTimer(3)
	while true do
		local e, key, cx, cy = iop.pullEvt()
		...
		elseif e == "mouse_scroll" then
			if key == -1 and scrolly > 0 then
				scrolly = scrolly - 1
				if cpu.clock() - scrollClock > 0.0005 then
					draw()
					gpu.cursPos(x - scrollx + offx, y - scrolly + offy)
				end
				scrollClock = cpu.clock()
				hasScrolled = true
		...
		elseif e == "timer" and key == tid then
			drawLine(y)
			tid = cpu.startTimer(3)
		end

		-- Draw
		if hasScrolled and cpu.clock() - scrollClock > 0.1 then
			draw()
			gpu.cursPos(x - scrollx + offx, y - scrolly + offy)
			hasScrolled = false
		end
		...
		-- Autosave
		if cpu.clock() - autosaveClock > autosaveInterval then
			saveFile(path, lines)
			autosaveClock = cpu.clock()
		end

		-- Errors
		if cpu.clock() - liveErrorClock > 1 then
...
local function update()
	local function draw(status)
		...
		gpu.setTxt(colors[theme.textColor])
		for i = 8, 10 do
			gpu.cursPos(w/2 - (status:len() + 4), i)
			write(string.rep(" ", status:len() + 4))
		end
		gpu.cursPos(w/2 - (status:len() + 4), 9)
		term.write(" - " .. status .. " ")
...
local function changeTheme()
	...
	if isAdvanced() then
		...
		iop.delete("/.LuaIDE_temp_theme_file")
		...
		if a then
			term.write("LuaIDE - Loaded Theme!")
			iop.delete(themeLocation)
			iop.move("/.LuaIDE_temp_theme_file", themeLocation)
			theme = a
			sleep(1.6)
			return "menu"
		end
...
centerPrint("Ported by Mr_Iron2")
